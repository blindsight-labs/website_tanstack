/* Hero — mockup 5.
   The render is the hero: an office network (rack, database, five desks) seen at
   a 3/4 angle, then seen, secured and governed (three/heroScene.ts). Text sits in
   the clear area on the left of an Octane-style inset sheet; a caption rail
   names the current beat and one audit-trail row seals along the bottom. */
...
export function Hero({ theme }: SectionProps) {
  ...
  useEffect(() => {
    ...
    import("./three/heroScene").then(async (mod) => {
      ...
      // chips follow their object; the status word changes found → contained → governed
      const setChips = () => {
